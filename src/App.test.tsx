...
describe('Stillboard core interactions', () => {
  ...
  it('generates an editable native diagram without persisting the API key', async () => {
    ...
    render(<App />)
    const canvas = await screen.findByLabelText('Infinite whiteboard canvas')
    fireEvent.click(screen.getByRole('button', { name: 'Open Stillboard AI' }))
    ...
    expect(screen.queryByLabelText('Stillboard AI')).toBeNull()
  })

  it('answers questions using an automatically attached board snapshot', async () => {
    const fetchMock = vi.fn(async () => ({ ok: true, json: async () => ({ answer: 'Add a second API gateway path for resilience.' }) }))
    vi.stubGlobal('fetch', fetchMock)
    render(<App />)
    const canvas = await screen.findByLabelText('Infinite whiteboard canvas')
    await chooseLibraryItem('Rectangle shape')
    fireEvent.pointerDown(canvas, { button: 0, pointerId: 60, clientX: 120, clientY: 140 })
    fireEvent.pointerMove(canvas, { buttons: 1, pointerId: 60, clientX: 300, clientY: 220 })
    fireEvent.pointerUp(canvas, { button: 0, pointerId: 60, clientX: 300, clientY: 220 })

    fireEvent.click(screen.getByRole('button', { name: 'Open Stillboard AI' }))
    fireEvent.change(screen.getByLabelText('OpenAI API key'), { target: { value: 'sk-test-user-key-12345678901234567890' } })
    expect(screen.getByText('Current board attached')).toBeTruthy()
    expect(screen.getByText('1 object')).toBeTruthy()
    fireEvent.change(screen.getByLabelText('Ask AI about this board'), { target: { value: 'Review this design' } })
    fireEvent.click(screen.getByRole('button', { name: 'Send question' }))
    expect(await screen.findByText('Add a second API gateway path for resilience.')).toBeTruthy()
    ...
    expect(request).toMatchObject({ mode: 'chat', question: 'Review this design', board: { name: 'Test board' } })
    expect(request.board.objects).toHaveLength(1)
    expect(canvas.querySelectorAll('[data-element-id]')).toHaveLength(1)
    expect(screen.getByLabelText('Stillboard AI')).toBeTruthy()
  })
})
